...
float suminvoicevalue=new InvoiceInterfaceImplementation().getSumInvoiceValue(date1, date2);
...
<th >InvoiceNo</th><th>Name</th><th>Gstin</th><th>Supply State</th><th>Date</th><th>InvoiceValue</th><th>Taxable Value</th><th>SgstRate</th><th>Cgst Rate</th><th>Sgst Amoutnt<th>Cgst Amount</th><th>Igst Rate</th><th>Igst Amount</th><th>Status</th>
</tr>



<%
List<InvoiceInfo> invoicelist= new InvoiceInterfaceImplementation().getInvoiceSummary(date1,date2);
if(invoicelist.size()!=0)
{
for(InvoiceInfo list:invoicelist)
{
%>
<tr><td><%=list.getInvoiceNO() %></td>
<td><%=list.getPartyName() %></td>
<td><%=list.getPartyGstin() %></td>
<td><%=list.getSupplyState() %></td>
<td><%=list.getDate() %></td>
<td><%=list.getInvoiceValue() %></td>
<td><%=list.getTaxableValue() %></td>
<td><%=list.getSgstRate() %></td>
<td><%=list.getCgstRate() %></td>
<td><%=list.getSgstAmt() %></td>
<td><%=list.getCgstAmt() %></td>
<td><%=list.getIgstRate()%></td>
