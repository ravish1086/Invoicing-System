...
public class InvoiceDataPurchase extends HttpServlet {
  
	private static final long serialVersionUID = 1L;
public InvoiceDataPurchase() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    for(int i=1;i<20;i++)
	    {
	    	...
	    	ItemSaleInfo info=new ItemSaleInfo(invoiceno,date,itemname,request.getParameter("itemhsn"+i),Float.parseFloat(request.getParameter("qty"+i)),Float.parseFloat(request.getParameter("amount"+i)),taxamount,Float.parseFloat(request.getParameter("itemtaxrate"+i)),Float.parseFloat(request.getParameter("amtaftrtax"+i)),Float.parseFloat(request.getParameter("amtaftrtax"+i)),request.getParameter("itemuom"+i));
	    	try {
				c = new ItemPurchaseInfoContr().setInfo(info);
	    ...
	    InvoiceInfo invoiceinfo4=new InvoiceInfo(invoiceno,invoiceno,partyname,partygstin,partysupplystate,date,taxablevalue28,invoicevalue,14f,14f,taxtwentyeight,taxtwentyeight,igstrate,igstamount,invoicestatus,partypan, partyphone, partyadd); 
	     boolean b1= new InvoiceInterfaceImplementationPurchase().createInvoice(invoiceinfo1);
	    boolean b2= new InvoiceInterfaceImplementationPurchase().createInvoice(invoiceinfo2);
	    boolean b3= new InvoiceInterfaceImplementationPurchase().createInvoice(invoiceinfo3);
	    boolean b4= new InvoiceInterfaceImplementationPurchase().createInvoice(invoiceinfo4);
	    boolean b5= new InvoiceInterfaceImplementationPurchase().createInvoice2(basicinfo);
	    
	    if((b1&&b2&&b3)||(b4&&b1&&b5))
	    {
	    	out.println("invoicegenerated");
	    RequestDispatcher rd=request.getRequestDispatcher("generatedinvoicepurchase.jsp");
